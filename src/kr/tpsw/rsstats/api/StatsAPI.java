...
public class StatsAPI {
	...
	public static void loadStatsItem() {
		...
		autosavecode = Bukkit.getScheduler().scheduleSyncRepeatingTask(Main.plugin, new Runnable() {
			public void run() {
	...
	public static void saveStatsItem() {
		Main.config.set("config.rpgexpsystem", rpgexpsystem);
		Main.config.set("config.levelup", levelup);
	}
	...
	public static void saveFirstStatsItem() {
		...
		Main.config.set("config.availablestats.item", "280");
		Main.config.set("config.availablestats.name", "§2[ §a남은 스텟 §2]");
		Main.config.set("config.availablestats.lore", list);
		Main.config.set("config.availablestats.x", 1);
		Main.config.set("config.availablestats.y", 3);

		list = new LinkedList<>();
		list.add("§8설정한 스텟을 저장합니다.");
		Main.config.set("config.save.item", "369");
		Main.config.set("config.save.name", "§2[ §a스텟 저장§2 ]");
		Main.config.set("config.save.lore", list);
		Main.config.set("config.save.x", 9);
		Main.config.set("config.save.y", 3);

		Main.config.set("config.rpgexpsystem", 0);
		Main.config.set("config.levelup", 1);//기본 설정 1업당 1스텟
		Main.config.set("auto_save_time", 600);
		Main.config.saveYaml();
	}// 첫 콘피그 저장
